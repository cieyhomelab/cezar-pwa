...
A mobile PWA (iPhone first, Android) for watching and steering a **Cezar** instance (`open-mercato/cezar`) running on the operator's host (the host is configuration: `<your-host>` / `$PUBLIC_ORIGIN` in the repo; `README.md` gives the reference deployment). The app is served from the same origin under `/m/`.
