...
1. **What exactly does the access perimeter look like in front of Cezar?** The mechanism
   is settled (a session issued from an access link), but three details are not: how long
   the session lasts, whether the access link already accepts a return parameter so the
   operator lands back inside the installed app, and what the product actually receives
   when the session is missing or stale — a redirect, a login page, or a bare refusal.
   Owner: the operator, by reading the live configuration. Blocks: FR-004 and FR-005 cannot
   be designed without it, and they gate the first milestone that shows real data.
...
3. **How does a permission request get answered?** Pending permission requests are the
   highest-priority attention state in the domain rule, yet the response mechanism has to
   be confirmed against Cezar's own cockpit before anything is built. These requests also
   only appear when an approval gate is switched on, which may mean this state never occurs
   on this instance in practice. Owner: the operator, by checking the instance's
   configuration. Blocks: whether the top-priority branch of the domain rule is reachable
   at all.
