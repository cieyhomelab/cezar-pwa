...
- **Q1** ~~Jak dokładnie działa ochrona cookie na VPS? (nginx `map` na cookie, `auth_request`, oauth2-proxy, Cloudflare Access…?) Czy link dostępowy przyjmuje parametr powrotu (`?next=`)? Jaki `Max-Age` ma cookie? → wpływa na R-AUTH-3/4.~~
  ...
  **Zostaje jedno pytanie do właściciela:** czy guard `if ($arg_key = …)` stoi w bloku `server` (działa dla każdej ścieżki, w tym `/m/`), czy wewnątrz `location /` (nigdy nie zobaczy `/m/`)? Od tego zależy, czy „Połącz z Cezarem” odblokowuje aplikację jednym wklejeniem (R-AUTH-3), czy zawsze schodzi na wariant zapasowy (R-AUTH-3a). Aplikacja działa w obu światach i sama rozpoznaje, w którym jest; **nic nie blokuje**. Sprawdzenie: wkleić prawdziwy link dostępowy w zainstalowanej PWA — jeśli po powrocie widać listę zadań, guard jest w `server`. Z repo tego nie da się sprawdzić: klucz wdrożeniowy to `command="/usr/bin/rrsync -wo /var/www"`, więc nie ma dostępu do `/etc/nginx`.
