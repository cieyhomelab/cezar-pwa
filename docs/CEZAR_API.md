...
- Wiersz: nazwa, `Enabled`/`Paused`, wyzwalacz (harmonogram słowami z domyślnymi wartościami kontraktu albo zdarzenia GitHuba), ostatnie uruchomienie (`lastRun`, dalej `state.lastRunAt`) z linkiem do zadania, następne (`nextRunAt`, tylko gdy włączona). `available: false` → notka z `reason` serwera dosłownie; lista zostaje (harmonogramy działają bez GitHuba).
