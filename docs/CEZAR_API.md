...
| Obrazek z transkryptu i załącznik z kolejki (#65) | `GET /api/v1/p/:projectId/runs/:id/images/:file` | bajty obrazu (lub pliku); `:file` to sama nazwa pliku, np. `2.png`. Nieprojektowy wariant `/api/v1/runs/:id/images/:file`, który serwer zapisuje w historii i w `queuedMessages[].images`, na żywym hoście (0.11.0) daje **404** |
...
- **Starsze strony (FR-049, #64):** gdy `hasOlder`, przewinięcie do góry transkryptu (`IntersectionObserver`, 300 px zapasu; ten sam przycisk „Show older entries” ręcznie) pobiera `GET …/history?cursor=<olderCursor>` i dokłada stronę **przed** trzymaną, w tym samym wpisie `['history', projectId, runId]` (`prependOlder()` w `src/domain/live-transcript.ts`). Składamy surowe linie obu stron scalone po `seq` (`mergeBySeq`), nie dwa złożone kawałki — tura przechodzi przez granicę stron, a deduplikacja v1/v2 działa w obrębie tury. Serwer powtarza linię otwierającą turę (`user-message`/`turn.started`) przed pierwszym elementem strony, choćby leżała dowolnie daleko wstecz — scalanie po `seq` ją zwija. Semantyka kursora z `dist/runs/event-history.js` @ `0.11.1`: `olderCursor` = base64url `{ v, kind: 'page', direction: 'older', fileSize, boundarySeq }`, strona starsza to ostatnie 100 elementów z `seq < boundarySeq`; plik krótszy niż `fileSize` → **409** (PWA traktuje jak każdy błąd — ponów albo cockpit). Kursor jest dla nas nieprzejrzysty.
  - Miejsce czytelnika zostaje (`useKeepPlace`): tuż przed zapisem do cache'u notujemy pierwsze widoczne wpisy (`data-entry-key` — klucz wpisu, nie tury, bo tura otwarta w połowie strony dostaje swoje wcześniejsze wpisy i może zmienić id), po renderze przewijamy o przesunięcie. Treść **nad** poznajemy po tym, że `pageReach()` (seq **drugiej** linii strony — pierwsza bywa powtórzoną linią otwierającą, wspólną dla stron jednej długiej tury) cofa się; wtedy `useFollowBottom` nie pokazuje „New messages”. Gdy przesuwa się do przodu (odświeżenie bez zakładki), to zwykła nowa treść — czytelnik na końcu podąża za nią.
  - Odświeżenie najnowszej strony (co 30 s bez strumienia, po zapisie) nie gubi tego, co przeczytano wstecz: `carryOver` zachowuje wcześniejsze linie i kursor wstecz, o ile świeża strona zachodzi na trzymaną (jej `pageReach()` ≤ `asOfSeq` trzymanej; trzymana sięga dalej, gdy jej `pageReach()` jest mniejszy — porównanie za linią otwierającą, bo w jednej długiej turze obie strony zaczynają się od tej samej). Bez zakładki świeża strona zastępuje wszystko, jak dawniej: linie pomiędzy nie leżą na żadnej, a pokazanie dziury jako ciągłości byłoby kłamstwem.
  - Błąd strony: w miejscu przycisku alert „Could not load older entries.” z „Retry” i odnośnikiem do cockpitu; transkrypt zostaje. Bez automatycznych ponowień. `hasOlder` bez `olderCursor` → dawny odnośnik do cockpitu. Na początku pliku (`hasOlder: false`) góra mówi „The transcript starts here.”, pod tym polecenie zadania.
  - Fixture'y: `transcript-long.ndjson` (syntetyczny, 1099 linii, pierwsza tura dłuższa niż strona) i `history-pages.long.json` — strony pocięte przez czytnik Cezara `0.11.1` (`scripts/record-history-pages.mjs`).
- **Obrazy z transkryptu (#65):** linia v1 `image` niesie `url` w nieprojektowej postaci `/api/v1/runs/:id/images/:file` (na żywym hoście 404) i `name` nadaną przez agenta. Z URL-a bierzemy **tylko nazwę pliku** (`^[A-Za-z0-9._-]+$`, bez samych kropek; `attachmentFileName()` w `apps/pwa/src/domain/run-images.ts`), nigdy ścieżkę, i czytamy ją z `…/p/:projectId/runs/:id/images/:file` (`runImageUrl()`). Miniatura otwiera się na pełnym ekranie; brak bezpiecznej nazwy albo błąd ładowania → dotychczasowa linia z nazwą. Ta sama reguła dla `queuedMessages[].images` (obrazy wg `isImageAttachmentName`, pozostałe pliki tylko z nazwy). CSP bez zmian (`img-src 'self'`). Obrazy w Markdownie agenta dalej nie są ładowane — renderujemy tekst alternatywny (żadnych żądań do stron trzecich).
...
| Anuluj auto-wznowienie (#63, FR-030) | `DELETE /api/v1/p/:projectId/runs/:id/auto-resume` | — → `{ cancelled: true }` (`cancelAutoResumeResponseSchema`, `z.literal(true)`). Idempotentna: run bez zaplanowanego wznowienia też odpowiada 200; odmawia tylko nieznany run, **404**. Zdejmuje `autoResumeAt`, więc zadanie staje się zwykłym `failed` (i wpada do „Wymaga uwagi” niezmienioną regułą). PWA oferuje ją tylko na `failed` z `autoResumeAt`, za potwierdzeniem |
